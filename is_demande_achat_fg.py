...
class is_demande_achat_fg(models.Model):
    ...
    fournisseur_id       = fields.Many2one('res.partner', 'Fournisseur', domain=[('is_company','=',True),('is_segment_achat.name','=',u'Fournisseurs de Frais généraux')])
    ...
    def create(self, vals):
        ...
        sequence_ids = data_obj.search([('name','=','is_demande_achat_fg_seq')])
        if sequence_ids:
            sequence_id = data_obj.browse(sequence_ids[0].id).res_id
            vals['name'] = self.env['ir.sequence'].get_id(sequence_id, 'id')
        obj = super(is_demande_achat_fg, self).create(vals)
        return obj
