...
CREATE OR REPLACE FUNCTION is_mini_liste_prix(product_product_id integer) RETURNS float AS $$
BEGIN
    RETURN (
        ...
        where ppi.product_id=product_product_id 
...
CREATE OR REPLACE view is_comparatif_lot_prix AS (
    select 
        ...
        is_mini_liste_prix(pp.id) as mini_liste_prix,
        ipc.lot_livraison         as lot_livraison,
        round(cast(ipc.lot_livraison      / is_mini_liste_prix(pp.id) as numeric),4)       as test_mini_liste_prix
    ...
    where is_mini_liste_prix(pp.id)!=0
          and (
            is_mini_liste_prix(pp.id)>ipc.lot_livraison 
            or
            round(cast(ipc.lot_livraison/is_mini_liste_prix(pp.id) as numeric),0) != round(cast(ipc.lot_livraison/is_mini_liste_prix(pp.id) as numeric),4) 
          )
)
